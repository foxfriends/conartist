    ...
    if (hoursInfo && hoursInfo.info) {
      ...
      if (todayOnly) {
        ...
        if (todayHours) {
          ...
          return (
            <Row
              title={<Font smallCaps>{l`Today's hours`}</Font>}
              value={l`${format(start)} - ${format(end)}`}
            />
          );
        }
      } else {
        const hoursText = newlinesToReact(
          hours
            .map(
              ([start, end]) =>
                l`${day(start)} ${format(start)} - ${format(end)}`,
            )
            .join("\n"),
        );
        return (
          <Row title={<Font smallCaps>{l`Hours`}</Font>} value={hoursText} />
        );
      }
    }
  } catch (_) {}
