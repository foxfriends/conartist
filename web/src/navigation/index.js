...
export class Navigation extends ReactX.Component {
  constructor(props) {
    ...
    const base = focused.pipe(
      takeUntil(this.unmounted),
      filter(() => !this.props.someAreDisabled),
      share(),
    );

    const loseFocus = base.pipe(
...
export class NavInfo {
  ...
  select(option, children, directness = DIRECT) {
    this.items = this.items.map((item) =>
      item
        .withChildren(item.title === option ? children : null)
        .select(item.title === option, directness),
    );
    return this;
  }
  ...
  enable(option, nested = false) {
    this.items = this.items.map((item) =>
      item.enable(!option || item.title === option, nested),
    );
    return this;
  }
  ...
  static forProductType({ id, name, discontinued }) {
    ...
    return [
      new ItemInfo(name, "remove", navigate.scrollTo("product-type", id)),
    ];
  }
  ...
  static forReorderableProductType({ id, name, discontinued }) {
    ...
    return [
      new ItemInfo(
        name,
        "remove",
        navigate.scrollTo("product-type", id),
      ).reorderable(),
    ];
  }
}
