...
impl Database {
    ...
    pub fn delete_product_type(&self,
        maybe_user_id: Option<i32>,
        type_id: i32,
    ) -> Result<bool, String> {
        let user_id = self.resolve_user_id_protected(maybe_user_id)?;
        ...
        conn.transaction(|| -> diesel::result::QueryResult<bool> {
                ...
                diesel::delete(products::table)
                    ...
                    .execute(&*conn)?;
                let products_left = diesel::select(dsl::exists(products::table.filter(products::type_id.eq(type_id))))
                ...
                if products_left {
                    ...
                    diesel::delete(producttypes::table
                        .filter(producttypes::type_id.eq(type_id)))
                        .execute(&*conn)?;
                    Ok(true)
                }
            })
            .map_err(|reason| format!("Could not delete product type with id {}. Reason: {}", type_id, reason))
    }
}
