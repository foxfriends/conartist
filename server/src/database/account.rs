...
impl Database {
    pub fn get_user_by_id(&self, maybe_user_id: Option<i32>) -> Result<User, String> {
        ...
        let user = if raw_user.email.is_some() {
            raw_user.unwrap()
        } else {
    ...
    pub fn get_user_for_email(&self, email: &str) -> Result<User, String> {
        ...
        let user = {
            let verified = users::table
                ...
                .map_err(|reason| {
                    format!(
                        "User with email {} could not be retrieved. Reason: {}",
                        email, reason
                    )
                })?;
    ...
    pub fn create_user(
        ...
        .map_err(|reason| {
            format!(
                "Could not create new user for {}. Reason: {}",
                email, reason
            )
        })
    }
    ...
    pub fn verify_email(&self, verification_code: &str) -> Result<User, String> {
        ...
        .map_err(|reason| {
            format!(
                "Could not verify email with code {}. Reason: {}",
                verification_code, reason
            )
        })
    }
    ...
    pub fn change_email(
        ...
        .map_err(|reason| {
            format!(
                "Could not change email of user with id {}. Reason: {}",
                user_id, reason
            )
        })
    }
    ...
    pub fn set_user_name(&self, maybe_user_id: Option<i32>, name: String) -> Result<User, String> {
        ...
        diesel::update(users::table)
            ...
            .map_err(|reason| {
                format!(
                    "Could not update name of user with id {}. Reason: {}",
                    user_id, reason
                )
            })
    }
    ...
    pub fn set_user_password(
        ...
        conn.transaction(|conn| {
            ...
            if !bcrypt::verify(&orig_password, &original).unwrap_or(false) {
                return Err(diesel::result::Error::DeserializationError(Box::new(
                    StringError("Original password is incorrect".to_owned()),
                )));
            }

            let hashed = bcrypt::hash(&new_password, bcrypt::DEFAULT_COST).map_err(|reason| {
                diesel::result::Error::DeserializationError(Box::new(StringError(format!(
                    "Couldn't hash password..? Reason: {}",
                    reason
                ))))
            })?;
        ...
        .map_err(|reason| {
            format!(
                "Could not update password of user with id {}. Reason: {}",
                user_id, reason
            )
        })
    }

    pub fn force_set_password(
        &self,
        ...
        .map_err(|reason| {
            format!(
                "Could not update password using code {}. Reason: {}",
                verification_code, reason
            )
        })
    }
    ...
    pub fn reset_password(&self, email: &str) -> Result<PasswordReset, String> {
        ...
        .map_err(|reason| {
            format!(
                "Could not reset password of user with email {}. Reason: {}",
                email, reason
            )
        })
    }
