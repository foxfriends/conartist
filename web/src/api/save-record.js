...
export class SaveRecord {
  ...
  constructor() {
    ...
    const addRecord = import(
      /* webpackChunkName: 'sales' */ "./graphql/mutation/add-record.graphql"
    );
    const updateRecord = import(
      /* webpackChunkName: 'sales' */ "./graphql/mutation/update-record.graphql"
    );
    ...
    this.deleteRecord = deleteRecord.then(
      (deleteRecord) => new GraphQLMutation(deleteRecord.default),
    );
  }
  ...
  send(action) {
    switch (action.action) {
      case "create": {
        ...
        return from(this.addRecord).pipe(
          flatMap((req) => req.send(variables)),
          map((response) =>
            response.state === "retrieved"
              ? {
                  state: "retrieved",
                  value: parse(response.value.addUserRecord),
                }
              : response,
          ),
        );
      ...
      case "update": {
        ...
        return from(this.updateRecord).pipe(
          flatMap((req) => req.send(variables)),
          map((response) =>
            response.state === "retrieved"
              ? {
                  state: "retrieved",
                  value: parse(response.value.modUserRecord),
                }
              : response,
          ),
        );
