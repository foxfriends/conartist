...
export class SignIn extends React.Component {
  ...
  trySignIn() {
    ...
    new SignInRequest(staySignedIn).send({ usr, psw }).subscribe((response) => {
    ...
    return (
      <Basic title={l`Sign in`} onClose={closeDialog}>
        <Form
          image={IMAGE}
          imageWidth={200}
          containerClassName={SS.padded}
          className={SS.form}
        >
          <div className={S.question}>{l`Welcome back`}</div>
          <Input
          ...
          <Checkbox
            ...
            defaultValue={staySignedIn}
            onChange={(staySignedIn) => this.setState({ staySignedIn })}
          >
            {l`Stay signed in`}
          </Checkbox>
          <div className={SS.signInFooter}>
            {passwordValidation.state === INVALID ? (
              <div className={SS.spacing}>
                <Tooltip title={passwordValidation.error}>
                  <Icon name="error" className={SS.error} />
                </Tooltip>
              </div>
            ) : (
              <div className={SS.spacing} />
          ...
          <Link
            className={SS.forgotPassword}
            onClick={() => showResetPasswordDialog(email)}
          >{l`I forgot my password`}</Link>
        </Form>
      </Basic>
    );
  }
