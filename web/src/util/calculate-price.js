...
export function calculatePrice(soldProducts, prices) {
  ...
  let items = soldProducts.reduce(
    (counts, product) => {
      const id = matters.includes(product.id)
        ? `product-${product.id}`
        : `product-type-${product.typeId}`;
      return counts.set(id, counts.get(id) + 1);
    },
    new DefaultMap([], 0),
  );
  return [...items].reduce((price, [key, count]) => {
    const relevantPrices = prices
      .filter(
        (price) =>
          (price.productId ? `product-${price.productId}` === key : false) ||
          `product-type-${price.typeId}` === key,
      )
      .sort(by(["quantity", Asc]));
    while (count > 0) {
      const bestPrice = relevantPrices.reduce((best, price) => {
        if (
