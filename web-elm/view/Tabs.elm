...
tabsWithFooter : List (Html msg) -> (Int -> msg) -> List (Html.Attribute msg) -> List (TabItem msg) -> Int -> Html msg
tabsWithFooter footerContents onSwitch attrs contents current =
  let position = \i ->
    if i < current then "positive"
    else if i > current then "negative"
    else "current"
