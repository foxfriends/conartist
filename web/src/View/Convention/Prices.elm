...
priceRow : Money.Currency -> PriceWithTypeAndProduct -> List (Html msg)
priceRow defaultCurrency { product, productType, price } =
  ...
  , div [ Table.cellLiner, tabularFigures ] [text (Price.normalize price |> Maybe.map .price |> Maybe.withDefault (Money.money 0) |> Money.resolveAuto defaultCurrency |> Money.prettyprint)] ]
