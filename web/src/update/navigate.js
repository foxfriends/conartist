...
export const discounts = goTo("/discounts", page.discounts);
export const editDiscounts = goTo("/discounts/edit", page.editDiscounts);
export const conventions = goTo("/conventions", page.conventions);
export const sales = goTo("/sales", page.sales);
export const searchConventions = goTo(
  "/conventions/search",
  page.searchConventions,
);
...
export function conventionDetails(convention) {
  ...
  if (
    currentPage.name !== "convention-details" ||
    currentPage.convention.id !== convention.id
  ) {
    window.history.pushState({}, "", `/convention/${convention.id}/details`);
  }
  loadConvention(convention.id).catch(conventions);
...
export function conventionRecords(convention) {
  const { page: currentPage } = model.getValue();
  if (
    currentPage.name !== "convention-records" ||
    currentPage.convention.id !== convention.id
  ) {
...
export function conventionStats(convention) {
  const { page: currentPage } = model.getValue();
  if (
    currentPage.name !== "convention-stats" ||
    currentPage.convention.id !== convention.id
  ) {
...
export function conventionUserInfo(convention) {
  const { page: currentPage } = model.getValue();
  if (
    currentPage.name !== "convention-user-info" ||
    currentPage.convention.id !== convention.id
  ) {
  ...
  model.next({
    ...model.getValue(),
    page: page.conventionUserInfo(convention),
  });
}
