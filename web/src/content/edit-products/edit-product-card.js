    ...
    >
      ...
      <Fragment>
        <List dataSource={products} reorderable={onProductReorder}>
          <div className={S.placeholder}>{l`No products yet... add one!`}</div>
          {(product, _, extraProps) => (
            <Item
              key={`product_${product.id}`}
              className={product.discontinued ? S.discontinued : ""}
              reorderable={!product.discontinued}
              {...extraProps}
            >
              <Input
              ...
              {product.discontinued ? null : (
                <Input
                  defaultValue={
                    isNaN(product.quantity) ? "" : `${product.quantity}`
                  }
                  placeholder={l`Quantity`}
                  onChange={(quantity) =>
                    onProductQuantityChange(product.id, quantity)
                  }
                  className={S.productQuantity}
                  validation={productQuantityValidation(
                    product.quantityValidation,
                  )}
                />
              )}
              <IconButton
              ...
              {product.discontinued ? (
                <IconButton
                  title="delete"
                  action={() => onProductDelete(product.id)}
