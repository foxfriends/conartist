...
export function ComingSoonConventionsCard({ conventions }) {
  return (
    <BasicCard
      title={
        <Font smallCaps regular>
          {(conventions.length === 1
            ? lx`{1} convention coming soon.`
            : lx`{${conventions.length}} conventions coming soon.`)(bold)}
        </Font>
      }
    >
      {conventions.length === 0 ? (
        ...
        <>
          ...
          <div className={S.findMore}>
            <Link onClick={navigate.searchConventions}>{l`Find more`}</Link>
          </div>
        </>
      )}
    </BasicCard>
