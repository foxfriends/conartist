...
export class ExportProducts extends React.Component {
  ...
  async doExport() {
    ...
    const files = (
      separateTypes
        ? [...separate.call(dataSource, "type")]
        : [["products", dataSource]]
    )
      .map(([name, file]) => [
        name,
        file.map((item) =>
          columns.map((column) => (item[column] || "").toString()),
        ),
      ])
        ...
        <div className={S.container}>
          <div className={S.options}>
            ...
            <div className={S.option}>
              <Checkbox
                defaultValue={separateTypes}
                onChange={(separateTypes) => this.setState({ separateTypes })}
              >
                {l`Separate types`}
              </Checkbox>
              <Tooltip
                ...
                title={
                  separateTypes
                    ? l`Product types will be split into different files`
                    : l`Product type will be included in each row`
                }
              >
            ...
            <div className={S.option}>
              ...
              <Tooltip
                className={S.info}
                title={l`Including IDs makes it much easier to import your products again later!`}
              >
                <Icon className={S.tooltip} name="info_outline" />
              </Tooltip>
            </div>
