...
export class ExportRecords extends React.Component {
  ...
  get dataSource() {
    ...
    const products = rawProducts
      .map(({ typeId, ...product }) => ({
        ...product,
        type: productTypes.find((type) => type.id === typeId),
      }))
      .map(({ type, ...product }) => ({
        ...product,
        type: type ? type.name : l`Unknown`,
      }));
    return [].concat(
      ...records
        // $FlowIgnore
        .map(({ products: sold, ...record }) => ({
          ...record,
          products: sold.map((id) =>
          ...
          switch (typeFormat) {
            ...
            case "Columns":
              const productsByType = new Map(
                [...separate.call(products, "type")].map(([type, products]) => [
                  type,
                  products.map(({ name }) => name).join("; "),
                ]),
              );
  ...
  async doExport() {
    ...
    const files = (
      separateDays
        ? [...separate.call(dataSource, "day")]
        : [["records", dataSource]]
    )
      .map(([name, file]) => [
        name,
        file.map((item) => columns.map((column) => item[column].toString())),
      ])
      .map(([name, file]) => [name, file.map((item) => item.join(","))])
      .map(([name, file]) => [
        name,
        includeTitles ? [columns.map(columnTitle).join(","), ...file] : file,
      ])
        ...
        <div className={S.container}>
          <div className={S.options}>
            ...
            <div className={S.option}>
              <Checkbox
                defaultValue={separateDays}
                onChange={(separateDays) => this.setState({ separateDays })}
              >
                {l`Separate days`}
              </Checkbox>
              <Tooltip
                ...
                title={
                  separateDays
                    ? l`Days will be split into different files`
                    : l`All records will be in one file`
                }
              >
            ...
            <div className={S.option}>
              <Checkbox
                defaultValue={includeInfo}
                onChange={(includeInfo) => this.setState({ includeInfo })}
              >
                {l`Include notes`}
              </Checkbox>
            </div>
            <div className={S.option}>
              <Checkbox
                defaultValue={includeTitles}
                onChange={(includeTitles) => this.setState({ includeTitles })}
              >
                {l`Include titles`}
              </Checkbox>
            </div>
            <div className={S.option}>
              <Select
                className={S.select}
                title={l`Product type format`}
                options={["Hidden", "Rows", "Columns", "Condensed"]}
                defaultValue={typeFormat}
                onChange={(typeFormat) => this.setState({ typeFormat })}
              >
          ...
          <div className={S.preview}>
            <Grid
              ...
              className={S.grid}
              style={{ width: 200 * columns.length }}
            >
              {(separateDays
                ? [...separate.call(dataSource, "day")]
                : [["", dataSource]]
                ...
                .map(([name, items]) => [
                  name,
                  includeTitles ? [columnTitles, ...items] : items,
                ])
                .map(([name, items]) => [
                  name ? (
                    <span
