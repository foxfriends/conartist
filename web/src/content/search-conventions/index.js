...
export class SearchConventions extends React.Component {
  ...
  loadConventions(fresh = false) {
    ...
    if (fresh || !isFull(conventions)) {
      ...
      this.conventionsConnection
        .send({ search: query, after: fresh ? null : conventions.endCursor })
        .pipe(
          filter(({ state }) => state === "retrieved"),
          pluck("value"),
          map((extension) =>
            fresh ? extension : extend(conventions, extension),
          ),
        )
        .subscribe((conventions) =>
          this.setState({ conventions, loading: false }),
        );
    }
  }
  ...
  render() {
      ...
      this.advancedSearch = (
        <SecondaryCard title={l`Advanced Search`}>
          <Input
            ...
            onChange={(country) =>
              this.setState({ country }, this.reloadConventions)
            }
          />
          <Input
            title={l`City`}
    ...
    return (
      <CardView
        dataSource={conventions.nodes}
        loadMore={
          !loading && !isFull(conventions) && !isEmpty(conventions)
