...
fn retrieve_convention(details_url: &reqwest::Url) -> Option<Convention> {
    ...
    let title =
        NAME_YEAR_REGEX.captures(&content(&details_document, &TITLE_SELECTOR)?)?[1].to_owned();
    let start_date = content(&details_document, &START_DATE_SELECTOR)?
        .parse()
        .unwrap();
    let end_date = content(&details_document, &END_DATE_SELECTOR)?
        .parse()
        .unwrap();
    let venue = content(&details_document, &VENUE_SELECTOR)?;
    let latitude = content(&details_document, &LATITUDE_SELECTOR)?
        .parse()
        .unwrap();
    let longitude = content(&details_document, &LONGITUDE_SELECTOR)?
        .parse()
        .unwrap();
...
fn main() {
    ...
    let table_url: reqwest::Url = args
        .next()
        .expect("A URL was not provided")
        .parse()
        .unwrap();
    ...
    for convention_link in document.select(&CONVENTION_SELECTOR) {
        let details_url = table_url
            .join(convention_link.value().attr("href").unwrap())
            .unwrap();
