...
const defaultToolbar = {
  primary: toolbarAction.SavePrices,
  secondary: toolbarAction.DiscardPrices,
};
...
export class EditPrices extends ReactX.Component {
  static getDerivedStateFromProps({ prices }, state) {
    if (!state || !state.hadPrices) {
      return {
        prices: prices
          .sort(
            by(["typeId", Asc], ["productId", Asc, Desc], ["quantity", Asc]),
          )
          .map(editablePrice),
  ...
  constructor(props) {
    ...
    const [savedPrices, savingPrices] = saveButtonPressed.pipe(
      map(() => diff(this.props.prices, this.state.prices)),
      switchMap(([prices, changes]) =>
        forkJoin(
          of(prices),
          forkJoin(...changes.map((price) => new SavePrice().send(price))).pipe(
            defaultIfEmpty([]),
          ),
        ),
      ),
    ...
    savedPrices
      .pipe(
        map(([prices, { value: changed }]) =>
          [].concat(
            prices,
            changed.filter((price) => price),
          ),
        ),
    ...
    const saveFailed = savingPrices.pipe(
  ...
  handleProductIdChange(id, productId) {
    const prices = this.state.prices.map((price) =>
      price.id === id ? { ...price, productId, original: null } : price,
    );
  ...
  handleQuantityChange(id, quantity) {
    const prices = this.state.prices.map((price) =>
      price.id === id ? { ...price, quantity: Number(quantity) } : price,
    );
  ...
  handlePriceChange(id, newPrice) {
    ...
    const prices = this.state.prices.map((price) =>
      price.id === id ? { ...price, price: money } : price,
    );
  ...
  validate(prices) {
    ...
    prices
      .map(hasher)
      .forEach((hash) =>
        existingPrices.set(hash, existingPrices.get(hash) + 1),
      );
    ...
    return prices.map((price) => {
      ...
      if (
        priceValidation.state === INVALID ||
        quantityValidation.state === INVALID
      ) {
        disableSave();
      }
  ...
  render() {
    ...
    const sortedPrices = prices.reduce(
      (sortedPrices, price) =>
        sortedPrices.set(price.typeId, [
          ...sortedPrices.get(price.typeId),
          price,
        ]),
      new DefaultMap([], []),
    );
          ...
          {([productType, prices]) => (
            <EditPricesCard
              ...
              products={products.filter(
                ({ typeId, discontinued }) =>
                  !discontinued && typeId === productType.id,
              )}
              prices={prices}
              bottomAction={addPrice(productType)}
              onProductChange={(priceId, productId) =>
                this.handleProductIdChange(priceId, productId)
              }
              onQuantityChange={(priceId, quantity) =>
                this.handleQuantityChange(priceId, quantity)
              }
              onPriceChange={(priceId, price) =>
                this.handlePriceChange(priceId, price)
              }
