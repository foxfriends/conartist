...
export async function starConvention(convention) {
  ...
  try {
    const response = await new StarConvention()
      .send({ conId: convention.id })
      .toPromise();
...
export async function unstarConvention(convention) {
  const { conventions: originalConventions, ...existingModel } =
    model.getValue();
  ...
  try {
    const response = await new UnstarConvention()
      .send({ conId: convention.id })
      .toPromise();
    if (response.state !== "retrieved") {
      throw new Error();
...
export function setConvention(convention) {
  ...
  const conventions = [...originalConventions].map((oldCon) =>
    oldCon.id === convention.id ? convention : oldCon,
  );
  model.next({
    ...model.getValue(),
