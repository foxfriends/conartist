...
export function ConventionStatsCard({ convention }) {
  ...
  if ((convention.records || []).length === 0) {
    return null;
  }

  const totalProfit = (convention.recordTotal || Money.zero).add(
    (convention.expenseTotal || Money.zero).negate(),
  );
    ...
    <Card>
      <BasicHeader>
        ...
        <Link
          ...
          priority="tertiary"
          onClick={() => navigate.conventionStats(convention)}
        >
      ...
      <Table>
        {profitPerHour ? (
          <MoneyInfo title={l`Profit per hour`} amount={profitPerHour} />
        ) : null}
      </Table>
    </Card>
  );
