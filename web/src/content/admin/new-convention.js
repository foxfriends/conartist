...
export class NewConvention extends React.Component {
  ...
  saveConvention() {
    const { startDate, endDate, hours, name, address, website, websiteURL } =
      this.state;
    ...
    new CreateConvention()
      ...
      .pipe(
        ...
        switchMap(({ value: conId }) =>
          forkJoin([
            new AddConventionInfo().send({
              conId,
              title: "Address",
              info: JSON.stringify(address),
              action: "TODO: coordinates",
              actionText: null,
            }),
            new AddConventionInfo().send({
              conId,
              ...
              info: JSON.stringify(
                hours.map((day) =>
                  day.map((time) =>
                    format(time || 0, "yyyy-MM-dd'T'HH:mm:ss.SSSX"),
                  ),
                ),
              ),
              action: null,
        ...
        map(batchResponses),
        tap(() => toast.show(<>Convention created</>)),
      )
      .subscribe();
  }
        ...
        hoursForm.push(
          <Fragment key={date.toString()}>
            <Input
              ...
              title="Start"
              type="time"
              onChange={(time) => this.handleTimeChange(index, "open", time)}
            />
            <Input
              className={S.halfInput}
              title="End"
              type="time"
              onChange={(time) => this.handleTimeChange(index, "close", time)}
            />
          </Fragment>,
        );
      }
      ...
      <BasicCard title="Add a convention" collapsible={true}>
        <div className={S.form}>
          ...
          <Input
            ...
            title="Start date"
            type="date"
            onChange={(startDate) => this.handleStartChange(startDate)}
          />
          <Input
            className={S.halfInput}
            title="End date"
            type="date"
            onChange={(endDate) => this.handleEndChange(endDate)}
          />
          <p>Address</p>
          ...
          <Input
            className={S.halfInput}
            title="URL"
            onChange={(websiteURL) => this.setState({ websiteURL })}
          />
          <p>Image (not yet supported)</p>
          <Button
            title="Save"
            className={S.button}
            action={() => this.saveConvention()}
          />
        </div>
      </BasicCard>
    );
