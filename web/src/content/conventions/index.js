    ...
    <CardView>
      ...
      {today.length === 0 ? (
        <Card className={S.emptyState}>
          <div className={S.placeholder}>{l`No conventions today`}</div>
        </Card>
      ) : null}
      ...
      {upcoming.length === 0 ? (
        <Card className={S.emptyState}>
          <div className={S.placeholderLarge}>
            {lx`You don't have any conventions starred... Why don't you go find one you like?`(
              (_) => _,
            )}
          </div>
        </Card>
      ...
      {completed.map((convention) => (
        <CompletedConventionCard
          showDetails
          convention={convention}
          key={`convention_${convention.id}`}
        />
