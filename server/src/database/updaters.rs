...
impl Database {
    ...
    pub fn update_expense(
        ...
        .map_err(|reason| {
            format!(
                "Could not update expense with id {}. Reason: {}",
                expense_id, reason
            )
        })
    }

    pub fn update_convention_user_info_vote(
        &self,
        maybe_user_id: Option<i32>,
        info_id: i32,
        approved: bool,
        ...
        conn.transaction(|| {
            diesel::insert_into(conventioninforatings::table)
                .values((
                    ...
                    conventioninforatings::con_info_id.eq(info_id),
                    conventioninforatings::rating.eq(approved),
                ))
                .on_conflict((
                    conventioninforatings::con_info_id,
                    conventioninforatings::user_id,
                ))
                .do_update()
            ...
            conventionuserinfo::table
                ...
                .select((
                    ...
                    dsl::sql::<sql_types::Integer>(
                        "SUM(CASE rating WHEN true THEN 1 ELSE 0 END)::INT",
                    ),
                    dsl::sql::<sql_types::Integer>(
                        "SUM(CASE rating WHEN false THEN 1 ELSE 0 END)::INT",
                    ),
                ))
        ...
        .map_err(|reason| {
            format!(
                "Could not change vote for user with id {} on info with id {}. Reason: {}",
                user_id, info_id, reason
            )
        })
    }
}
