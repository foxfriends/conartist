...
import { ProductTypes, Products, Prices, Record, Con } from './src/types';
...
const getCons = (() => {
  let cons: { [key: string]: Con } | null = null;
  return async (): Promise<{ [key: string]: Con }> => {
    if(!cons) {
      cons = JSON.parse(await readFile('cons.json')) as { [key: string]: Con };
    }
    return cons;
  };
})();
...
app.get('/con-info/:con', async (req, res) => {
  ...
  // TODO: determine if user has permissions
  res.header('Content-Type: application/json');
  res.send(JSON.stringify(data));
});

app.get('/dashboard/cons', async (_, res) => {
  res.header('Content-Type: application/json');
  res.send(await readFile('data/cons.json'));
});
