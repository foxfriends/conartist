...
impl Database {
    pub fn get_suggestions(
        ...
        limit: i64,
        after: Option<&String>,
    ) -> Result<Vec<ScoredSuggestion>, String> {
        ...
        if let Some(status) = status {
            scoredsuggestions::table
                ...
                .load::<ScoredSuggestion>(&mut conn)
    ...
    pub fn create_suggestion(&self, suggestion: String) -> Result<ScoredSuggestion, String> {
        ...
        diesel::insert_into(suggestions::table)
            .values((
                suggestions::user_id.eq(user_id),
                suggestions::suggestion.eq(suggestion),
            ))
            .get_result::<Suggestion>(&mut conn)
            .map(|suggestion| suggestion.with_score(0))
            .map_err(|reason| {
                format!(
                    "Failed to create new suggestion as user with id {}. Reason: {}",
                    user_id, reason
                )
            })
    }
    ...
    pub fn vote_for_suggestion(&self, suggestion_id: i32) -> Result<ScoredSuggestion, String> {
        ...
        conn.transaction(|conn| {
            diesel::insert_into(suggestionvotes::table)
                .values((
                    suggestionvotes::suggestion_id.eq(suggestion_id),
                    suggestionvotes::user_id.eq(user_id),
                ))
                .execute(conn)?;
            scoredsuggestions::table
        ...
        .map_err(|reason| {
            format!(
                "Could not vote as user with id {} on suggestion with id {}. Reason: {}",
                user_id, suggestion_id, reason
            )
        })
    }
}
